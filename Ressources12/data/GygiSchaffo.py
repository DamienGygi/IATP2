...
def mutation():
    for i in range(0,4):
        if (random.random() < 0.8):
            ...
            r1=random.randint(1,len(indiv.travelPath)-2)
            r2=random.randint(1,len(indiv.travelPath)-2)
            first = listOrder.pop(r1)
            last = listOrder.pop(r2)
            listOrder.insert(r1, last)
            listOrder.insert(r2, first)
